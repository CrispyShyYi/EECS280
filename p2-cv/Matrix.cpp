...
int Matrix_row(const Matrix* mat, const int* ptr) {
  int row = (ptr - &(mat->data[0])) / mat->width;
  return row;
}
...
int Matrix_column(const Matrix* mat, const int* ptr) {
  int column = (ptr - &(mat->data[0])) % mat->width;
  return column;
}
...
int Matrix_max(const Matrix* mat) {
  ...
  for (int i = 0; i < mat->height; i++){
    for (int j = 0; j < mat->width; j++){
      if (maxValue < mat->data[i * mat->width + j]){
        maxValue = mat->data[i * mat->width + j];
      }
    }
  }
  return maxValue;
...
int Matrix_column_of_min_value_in_row(const Matrix* mat, int row,
  ...
  for(int i = column_start + 1; i < column_end; i++){
    if (minValue > mat->data[row * mat->width + i]){
      minColumn = i;
      minValue = mat->data[row * mat->width + i];
    }
  }
  return minColumn;
}
